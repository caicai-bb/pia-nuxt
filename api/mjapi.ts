...
import { gptServerStore, homeStore, useAuthStore, useChatStore } from '@/store'
...
export function upImg(file: any): Promise<any> {
  const maxSize = homeStore.myData.uploadImgSize ? (+homeStore.myData.uploadImgSize) : 5
...
export function upLoadImg(file: any): Promise<any> {
  const formData = new FormData()
  formData.append('files', file)
  formData.append('agentId', homeStore.myData.agentId)
  // try {
  return post({
    url: '/ai-api/ai/agent/uploadFile',
    data: formData,
    headers: {
      //   'accept': '/',
...
export function storageUpload(file: any): Promise<any> {
  const formData = new FormData()
  formData.append('files', file)
  // formData.append('agentId', homeStore.myData.agentId)
  // try {
  return post({
    url: '/ai-api/ai/storage/upload',
    data: formData,
    headers: {
      //   'accept': '/',
      'Content-Type': 'multipart/form-data',
...
export const flechTask = (chat: Chat.Chat) => {
  ...
  const check = async () => {
    ...
    if (opt.progress && opt.progress == '100%')
      chat.loading = false
    if (typeof chat.uuid === 'number') {
      if (ts?.task?.status === 'success') {
        // 如果是当前新建的会话 同步接口id
        useChatStore().upChatUuid(chat.uuid, ts.task.id)
        useChatStore().updateHistory(chat.uuid, { uuid: ts.task.id })
        useChatStore().setActive(ts.task.id)
        if (ts.task?.outputFileUrl) {
          const index = useChatStore().history.findIndex(item => item.uuid === ts.task.id)
          if (index !== -1) {
            if (!useChatStore().history[index].imageUrl)
              useChatStore().updateHistory(ts.task.id, { imageUrl: ts.task.outputFileUrl })
          }
        }
      }
    }
    if (opt.code == 23) { chat.text = opt.description }
    else if (ts.task.status === 'failed') {
      chat.mjID = ''
      chat.text = 'There was an error there, please try again'
      chat.loading = false
      homeStore.setMyData({ act: 'updateChat', actData: chat })
    }
    homeStore.setMyData({ act: 'updateChat', actData: chat })
    // "NOT_START" //["SUBMITTED","IN_PROGRESS"].indexOf(ts.status)>-1
    if (!['FAILURE', 'SUCCESS', 'success', 'failed'].includes(ts?.task?.status) && cnt < 100) { setTimeout(() => check(), 5000) }
    else {
...
export const getActionData = async (res: any, data: any, chat: Chat.Chat) => {
  let cnt = 0
  let stop = false
  cnt++
  if (!res.taskId) {
  ...
  const ts = await mjFetch('/ai-api/ai/task/get', params)
  const opt = ts?.task?.output ? JSON.parse(ts?.task.output) : {}

  if ((data.action && data.action == 'CustomZoom' && opt.result)
    || (data.action && data.action == 'mask' && opt.result)
    || (data.action && data.action == 'changeV2' && (opt.description && opt.description.includes('confirm')))
    || opt?.code == 21) {
    const dt = { ...data, actionOk: 'submitModal', taskId: opt.result }
    subTask(dt, chat)
    http.getUserTokenAccount()
    stop = true
  }
  else if (opt?.id) {
    // 如果没有code 有id需要显示 没id继续get
    const d = { code: 1, taskId: ts.task.id }
    backOpt(d, chat)
    stop = true
  }
  else if (ts.task.status === 'failed') {
  ...
  if (!['FAILURE', 'SUCCESS', 'success', 'failed'].includes(ts?.task?.status) && cnt < 100 && !stop)
    setTimeout(() => getActionData(res, data, chat), 5000)
}
export const subTask = async (data: any, chat: Chat.Chat) => {
  ...
  try {
    // return ;
    if (data.actionOk && data.actionOk == 'submitModal') { // model
      const bdata = {
        ...
        productNo: 'pia',
        data_request: {
          maskBase64: data?.maskData?.maskBase64,
          prompt: data?.maskData?.prompt,
          taskId: data.taskId,
          model: 'mj-zeakai-modal',
        },
      }
      d = await mjFetch('/ai-api/ai/zeakai/mj/submit/modal', bdata)
    }
    else if (data.action && data.action == 'change') { // 执行变化
      d = await mjFetch('/mj/submit/change', data.data)
    }
    else if (data.action && data.action == 'CustomZoom') { // 自定义变焦
      d = await mjFetch('/ai-api/ai/zeakai/mj/submit/action', data.data)
      // if (d.result) {
      ...
      // }
      if (d.code === 1)
        return getActionData(d, data, chat)
    }
      ...
      d = await mjFetch('/ai-api/ai/zeakai/mj/submit/describe', data.data)
    }
      ...
      const toData = {
        ...
        image_request: {
          ...
          // modes: [],
          botType: 'MID_JOURNEY',
          model: 'mj-zeakai-imagine',
        },
      }
      if (data.bot && data.bot == 'NIJI_JOURNEY')
        toData.botType = data.bot

      d = await mjFetch('/ai-api/ai/zeakai/mj/submit/imagine', toData)
      mlog('submit', d)
      // return ;
    }
    // mlog("subTask rz >> ", d )
    ...
    // d = await mjFetch('/ai-api/ai/zeakai/mj/submit/modal', { taskId: d.result })
    else if (d.code == 1) {
      backOpt(d, chat)
    }
    else if (d.code == 0) {
...
const backOpt = (d: any, chat: Chat.Chat) => {
  if (d.code == 1) {
    chat.text = d.code == 22 ? d.description : 'Please wait！'
    chat.mjID = d.taskId
    flechTask(chat)
    chat.loading = true
    homeStore.setMyData({ act: 'updateChat', actData: chat })
...
export const canVisionModel = (model: string) => {
  ...
  // const mList = ['GPT-4o mini', 'o1-preview', 'GPT-4o', 'GPT4', 'Claude3.5-Sonnet', 'Gemini 2.0 Flash']
  const mList = []
  mList.forEach((m) => {
    if (m === model)
      return true
...
export const isCanBase64Model = (model: string) => {
  // gpt-4o
  // customVisionModel
  const arr = ['Gemini 2.0 Flash', 'Claude3.5-Sonnet', 'GPT4', 'GPT-4o', 'o1-preview', 'GPT-4o mini', 'DeepSeek']
  for (const m of arr) {
    if (model === m)
      return true
  }
