    ...
    <NModal v-model:show="show" :auto-focus="false" preset="card" style="width: 95%; max-width: 720px">
      <div class="c-r-a">
        ...
        <div :class="[isMobile ? 'mt-4' : 'c-r-a-w']">
          <div
            v-for="item in textList" :key="item"
            :class="[isMobile ? ' text-xl font-semibold text-left px-4 text-white' : 'c-r-a-text']"
          >
            {{ item }}
          </div>
        </div>
        <div :class="[isMobile ? 'mb-4 text-right' : 'c-r-a-btn']">
          <div
            v-if="backText" :class="[isMobile ? 'border border-[#fe5997] border-solid rounded-2xl px-4 mt-4 py-1 text-center text-lg inline-block mr-4'
              : 'crabtn-div crd-back']" @click="confirm(backText)"
          >
            {{ backText }}
          </div>
          <div
            :class="[isMobile ? 'bg-[#fe5997] rounded-2xl px-4 mt-4 py-1 text-center text-lg inline-block mr-4'
              : 'crabtn-div crd-confirm']"
